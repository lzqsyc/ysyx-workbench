...
| `|` | 逻辑或 | `cat|dog` |
| `\` | 转义 | `\.` 匹配 `.` |
| `{n}` | 前项恰好重复 n 次 | `a{3}` → `aaa` |
| `{n,}` | 前项至少重复 n 次 | `a{2,}` → `aa`、`aaa` |
| `{n,m}` | 前项重复 n~m 次 | `a{2,4}` → `aa`、`aaa`、`aaaa` |

#### 基础正则与扩展正则区别

- **基础正则（BRE）**：如 `grep` 默认模式，`+`、`?`、`|`、`{}` 需加反斜杠转义。
- **扩展正则（ERE）**：如 `egrep`/`grep -E`/`awk`，上述元字符无需转义。

#### 常用字符类

| 表达式 | 说明 |
| :--- | :--- |
| `\d` | 数字，等价于 `[0-9]`（部分工具支持，如 Perl/PCRE）|
...
> 注：`\d`、`\w`、`\s` 等在 `grep -P`、Perl、Python 等支持 PCRE 的工具中可用。

#### 常见正则表达式案例

- 匹配邮箱：`[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}`
- 匹配手机号（中国）：`1[3-9][0-9]{9}`
- 匹配 IPv4 地址：`([0-9]{1,3}\.){3}[0-9]{1,3}`
- 匹配 16 进制数：`0[xX][0-9a-fA-F]+`
- 匹配空行：`^\s*$`

#### grep/sed/awk 常用正则选项

- `grep -E`：扩展正则（等价于 `egrep`）
- `grep -P`：Perl 正则（支持 `\d`、`(?=...)` 等高级语法）
- `sed -E`：扩展正则
- `awk`：天然支持扩展正则
